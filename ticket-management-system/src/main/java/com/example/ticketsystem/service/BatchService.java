...
public class BatchService {
	...
	public BatchProcess createProcess(String fileName) {
		BatchProcess process = new BatchProcess(fileName);
...
//	}
	...
	private boolean isHeaderRow(Row row) {
		...
		return cellValue != null && (cellValue.equalsIgnoreCase("título") || cellValue.equalsIgnoreCase("titulo")
	...
	public long getTotalProcessedRecords() {
		return batchRepository.findAll().stream().mapToInt(BatchProcess::getProcessedRecords).sum();
	}

	public long getTotalSuccessRecords() {
		return batchRepository.findAll().stream().mapToInt(BatchProcess::getSuccessCount).sum();
	}
